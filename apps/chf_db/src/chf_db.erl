...
-moduledoc "Facade for the pluggable document store. Dispatches to the backend module\n"
           "configured via the `{chf_db, backend}` app env (cached in persistent_term).\n"
           "\n"
...
-doc "Resolve (and cache) the configured backend module. Defaults to `chf_db_mnesia`.\n"
     "Cached in persistent_term on first call, so changing the `{chf_db, backend}`\n"
...
-spec find_by(chf_db_backend:collection(), chf_db_backend:index(), term()) ->
    {ok, [chf_db_backend:doc()]} | {error, term()}.
...
-spec update(chf_db_backend:collection(), chf_db_backend:key(),
             fun((chf_db_backend:doc()) -> {ok, chf_db_backend:doc()} | {abort, term()})) ->
    {ok, chf_db_backend:doc(), chf_db_backend:version()} |
    {error, {aborted, term()}} |
    {error, not_found} |
    {error, max_retries} |
    {error, term()}.
...
-doc "Insert-if-absent. Returns `{ok, Version}` on success, `{error, exists}` if a\n"
     ...
     "Callers should hold a per-key cluster lock when strict uniqueness is required.".
-spec create(chf_db_backend:collection(), chf_db_backend:key(), chf_db_backend:doc()) ->
    {ok, chf_db_backend:version()} | {error, exists} | {error, term()}.
create(Coll, Key, Doc) ->
    case (backend()):get(Coll, Key) of
        {ok, _, _}         -> {error, exists};
...
ready() ->
    case backend() of
        chf_db_mnesia ->
            ...
            case chf_db_mnesia:wait_ready_timeout(Tables, 0) of
                ok -> true;
                _  -> false
            end;
        _ ->
...
await_ready(Timeout) ->
    case backend() of
        chf_db_mnesia ->
            Tables = mnesia:system_info(local_tables) -- [schema],
            chf_db_mnesia:wait_ready_timeout(Tables, Timeout);
        _ ->
            ok
    end.
