...
%% @doc Generate a cluster-unique chargingDataRef.
%% Node-tagged so refs from different cluster nodes never collide.
%% Any node can resolve a ref generated by another node because the session
%% lives in the shared replicated store keyed by this ref.
-spec generate_ref() -> binary().
generate_ref() ->
    Time   = erlang:system_time(microsecond),
    Unique = erlang:unique_integer([positive, monotonic]),
    iolist_to_binary([atom_to_binary(node(), utf8), $-,
                      integer_to_binary(Time), $-, integer_to_binary(Unique)]).
