...
%% chf_sbi_offline_SUITE.erl — CT integration tests for Nchf_OfflineOnlyCharging handler.
-module(chf_sbi_offline_SUITE).
-compile(export_all).
...
update_session_not_found(Config) ->
    ...
    {Status, _RespHeaders, _RespBody} = post_json(ConnPid,
        "/nchf-offlineonlycharging/v1/offlinechargingdata/nosuchref/update", Body),

    ?assertEqual(404, Status).

%% TS 32.291 §6.2 allows an empty body on release (final accounting may have
%% nothing to report). The handler currently reads the body and rejects <<>> as
%% 400 — this is stricter than the spec. NOTE: gap — TS 32.291 §6.2.3 permits
%% an empty ChargingDataRequest body on release; the handler should treat an
%% empty body as an empty multipleUnitUsage list and return 204, not 400.
release_empty_body(Config) ->
    ConnPid = ?config(conn, Config),
    Headers = [{<<"content-type">>, <<"application/json">>}],
    StreamRef = gun:post(ConnPid,
        "/nchf-offlineonlycharging/v1/offlinechargingdata/ref456/release",
        Headers, <<>>),
    {response, _Fin, Status, _RespHeaders} = gun:await(ConnPid, StreamRef),
    %% Current behavior: 400. NOTE: TS 32.291 §6.2.3 allows an empty release
    %% body; this should ideally be 204 (or at least 200) per the spec.
    ?assertEqual(400, Status).
...
wrong_content_type(Config) ->
    ...
    meck:expect(chf_core, session_update, fun(_, _) ->
        {ok, #{1 => #{granted => 0, outcome => granted}}} end),
    Body = json:encode(#{<<"multipleUnitUsage">> => []}),
    Headers = [{<<"content-type">>, <<"text/plain">>}],
    StreamRef = gun:post(ConnPid,
        "/nchf-offlineonlycharging/v1/offlinechargingdata/ref456/update",
        Headers, Body),
